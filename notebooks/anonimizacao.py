...
# Nota: .parent.parent é necessário aqui pois o script está dentro de 'notebooks/'
BASE_DIR = Path(__file__).resolve().parent.parent
INPUT_FILE = BASE_DIR / 'data' / 'all_dti_processed.csv'
OUTPUT_FILE = BASE_DIR / 'data' / 'all_dti_anonymized.csv'
CONFIG_FILE = BASE_DIR / 'config_anonimizacao.json'  # Caminho do arquivo de config

# Carregar termos sensíveis globalmente para evitar I/O repetitivo
SENSITIVE_TERMS = []
try:
    if CONFIG_FILE.exists():
        with open(CONFIG_FILE, 'r', encoding='utf-8') as f:
            config_data = json.load(f)
            SENSITIVE_TERMS = config_data.get('sensitive_terms', [])
    else:
        print(
            f"AVISO: Arquivo de configuração '{CONFIG_FILE}' não encontrado. A limpeza de termos específicos será ignorada.")
...
def scrub_text(text):
    ...
    # Utiliza a lista carregada do arquivo JSON
    if SENSITIVE_TERMS:
